...
const onSelectionChanged = () => {
    ...
    if (selectedRows.length > 0) {
        codeItems.value = data.filter(rec => rec.codeTy === selectedRows[0].cmmnCode).sort((a, b) => (String(a.ordr).padStart(10, "0") + a.cmmnCode).localeCompare(String(b.ordr).padStart(10, "0") + b.cmmnCode));
    }
};
...
const searchItems = (event) => {
    ...
    for (let i = 0; i < codeTypeItems.value.length; i++) {
        ...
        if (item.cmmnCodeNm.toLowerCase().indexOf(query.toLowerCase()) >= 0) {
            _filteredItems.push(item.cmmnCodeNm);
        }
    }
