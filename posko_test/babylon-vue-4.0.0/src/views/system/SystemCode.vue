        ...
        <div class="card formgrid grid" style="margin-bottom: 0px;">
            <div class="field col grid" style="margin-bottom: 0px;">
                ...
                <div class="field  col-12 md:col-6" style="margin-bottom: 0px;">
                    ...
                    <SelectButton v-model="useAt" :options="options" aria-labelledby="basic" class="w-full" @click="setData(data)"/>
                </div>
            </div>
...
const columnsType = [
    {
        ...
        field: "cmmnCode",
        editable: false,
        cellStyle: ags.agGridDefaultCellStyle
    },
    {
...
const onSelectionChanged = () => {
    ...
    selectCodeTy = selectedRows[0].cmmnCode;
};
...
const setData = (responseData) => {
    ...
    //코드타입 데이타만 업데이트
    let sUseAt = useAt.value==="전체"?"Y,N":useAt.value==="사용"?"Y":"N";
    codeTypeItems.value = data.filter(rec => rec.codeTy === '000' && sUseAt.indexOf(rec.useAt) > -1).sort((a, b) => (String(a.ordr).padStart(10, "0") + a.cmmnCode).localeCompare(String(b.ordr).padStart(10, "0") + b.cmmnCode)) ;
...
const saveData = async () => {   
    ...
    try{    
        ...
        let updatedData = codeItems.value.filter(rec => (rec.rowStatus != null && rec.rowStatus != ''));
        ...
        const response = await axios.post('/api/code/saveCmmncode', saveParams);
        console.log('response => ');
        console.log(response);
